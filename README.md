...
**Менеджер закладок браузера.**<br>Для быстрого наведения порядка.
</div>
...
```
**Открытие проекта и запуск из исходников**<br>Проект использует пакетный менеджер `uv` для удобной организованности.
```bash
...
```
**Как добавить новый язык интерфейса**<br>Структура словаря `*.json`
```bash
{
...
**Сборка в EXE**<br>Для автономности сборка производиться пакетом `nuitka`.
```bash
cd Markhive
uv add nuitka
build.bat
